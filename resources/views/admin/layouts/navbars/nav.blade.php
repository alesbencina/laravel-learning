            ...
            <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4 d-flex justify-content-end" id="navbar">
                <ul class="navbar-nav justify-content-end">
                    <li class="nav-item d-flex align-items-center">
                        @if(auth()->check())
                            <span class="text-xs text-secondary mb-0 px-3">Logged in: {{ auth()->user()->name }}</span>
                            <form method="POST" action="/logout" class="text-xs font-semibold text-blue-500 ml-5">
                                @csrf
                                <button type="submit" class="btn bg-gradient-dark btn-md mt-4 mb-4">Log out</button>
                            </form>
                        @endif
                    </li>
                </ul>
